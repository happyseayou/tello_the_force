    ...
    def new_iter(self, values):
        ...
        if self.iter > 1:
            ...
            for i in range(self.nb_values):
                cv2.line(self.canvas, ((self.iter-1)*self.step_width, int(self.height-self.prev_values[i]*self.height/(self.y_max-self.y_min))), (
                    self.iter*self.step_width, int(self.height-values[i]*self.height/(self.y_max-self.y_min))), self.colors[i], self.thickness[i])
            if self.threshold:
                cv2.line(self.canvas, (0, int(self.height-self.threshold*self.height/(self.y_max-self.y_min))),
                         (self.width, int(self.height-self.threshold*self.height/(self.y_max-self.y_min))), (0, 255, 0), 1)
...
class FPS:  # 这个模块摘自tello_openpose
    ...
    def display(self, win, orig=(10, 30), font=cv2.FONT_HERSHEY_PLAIN, size=2, color=(0, 255, 0), thickness=2):
        cv2.putText(win, f"FPS={self.get():.2f}",
                    orig, font, size, color, thickness)
...
class Pydisplay():
    def __init__(self):
        ...
        if self.isdisplay == 1:
            self.screen = pygame.display.set_mode(
                (640, 480), pygame.DOUBLEBUF)  # 键盘控制封不了类，只能用函数
    ...
    def display(self, image2surface, flightstate):
        if self.isdisplay == 1:
            ...
            # 滚动俯
            newroll = pygame.transform.rotate(
                self.roll, -(180/pi)*atan(flightstate[15]/5))  # 使用响应曲线放大
            newrect = newroll.get_rect(center=self.rollrect.center)
            self.screen.blit(
                newroll, (newrect[0]+168, newrect[1]+115-(366/pi)*atan(flightstate[16]/5)))  # 使用响应曲线放大
            # 电池高度速度
            ...
            self.screen.blit(
                newyawp, (newyawprect[0]+74, newyawprect[1]+368))  # 使用响应曲线放大
            # 飞行模式和灯
    ...
    def usec(self, key_list):  # 定义键盘跟踪，暂时没有封转到类
        ...
        if key_list[pygame.K_F12]:
            pygame.display.set_mode(
                (640, 480), pygame.FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF)
        # 特殊指令执行时通道指令无效且归零
...
class UID():  # 显示类
    ...
    def show(self, image, kp, flightstate):
        if kp == 0:  # 两种显示模式
            ...
            # PID调节文件
            if self.pidt:
                self.pidfilewrite.writerow(
                    [flightstate[2], flightstate[8], flightstate[9], flightstate[13], flightstate[14]])
        cv2.imshow('tello', image)
    ...
    def drawer(self, image, kp, flightstate):
        ...
        if x11 and x22 and y11 and y22:
            cv2.arrowedLine(image, (x11, y11), (x22, y22),
                            (0, 0, 255), 5, 8, 0, 0.1)
    ...
    def hubw(self, image, flightstate):
            ...
            def draw(self, frame):
                ...
                for (info, color) in self.infos:
                    cv2.putText(frame, info, (0, 30 + (i * 30)),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)  # lineType=30)
                    i += 1
        ...
        # 四元数解算的旋转是与启动点为原点的，可用于定位
        hud.add(f"zero pitch {flightstate[22]:8.1f}")
...
class Mapui:
    def __init__(self):
        ...
        self.pidt = 0
        if self.pidt:
            self.yawpid = RollingGraph(window_name="yawpid", thickness=[
                                       1], threshold=125, waitKey=False)
            self.thropid = RollingGraph(window_name="thropid", thickness=[
                                        1], threshold=125, waitKey=False)
            self.pithpid = RollingGraph(window_name="pithpid", thickness=[
                                        1], threshold=125, waitKey=False)
            self.rollpid = RollingGraph(window_name="rollpid", thickness=[
                                        1], threshold=125, waitKey=False)
    ...
    def drawmap(self, img, flightstate):
        ...
        for item in self.lspoint:
            coloris = (random.randint(0, 255), random.randint(
                0, 255), random.randint(0, 255))
            cv2.circle(img, (item[0], item[1]), 5, coloris, -1)
        # 画飞机指向箭头
        if flightstate[14] == 0:
            arrowed2len = 40
            arrowedstart = (int(x+arrowed2len*sin(-radians(flightstate[23]))), int(
                y+arrowed2len*cos(-radians(flightstate[23]))))
            arrowedpoint = (int(x-arrowed2len*sin(-radians(flightstate[23]))), int(
                y-arrowed2len*cos(-radians(flightstate[23]))))
            cv2.arrowedLine(img, arrowedstart, arrowedpoint,
                            (255, 0, 0), 2, 1, 0, 0.05)
        else:
            arrowed2len = 40
            arrowedstart = (int(x+arrowed2len*sin(-radians(flightstate[24]))), int(
                y+arrowed2len*cos(-radians(flightstate[24]))))
            arrowedpoint = (int(x-arrowed2len*sin(-radians(flightstate[24]))), int(
                y-arrowed2len*cos(-radians(flightstate[24]))))
            cv2.arrowedLine(img, arrowedstart, arrowedpoint,
                            (255, 0, 0), 2, 1, 0, 0.05)
