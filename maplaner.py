...
cv2.putText(img, guide1, (50, 350), cv2.FONT_HERSHEY_SIMPLEX,
            1.8, (255, 255, 255), 2)
preimg = None
nowimg = img
prelist = []
    ...
    if raiseerror == 0:
        if flags == 1 and event == cv2.EVENT_LBUTTONDBLCLK:  # 左键双击go
            if ls != [] and ls[len(ls)-1][0] != 4 and inputing != True:
                ...
                cv2.rectangle(nowimg, (x+20, y-10),
                              (x+60, y-40), (255, 255, 255), 1)
                ls.append([2, x, y, 0])
                lscom.append([2, posx, posy, posz])
                i = len(ls)-2
                if ls[i+1][0] != 4 and ls[i+1][0] != 1:
                    cv2.arrowedLine(nowimg, (ls[i][1], ls[i][2]), ((
                        ls[i+1][1]+ls[i][1])//2, (ls[i+1][2]+ls[i][2])//2), (0, 255, 255), 2, 1, 0, 0.05)
                    cv2.line(nowimg, ((ls[i+1][1]+ls[i][1])//2, (ls[i+1][2]+ls[i]
                                                                 [2])//2), (ls[i+1][1], ls[i+1][2]), (0, 255, 255), 2, 4)
            ...
            if ls != [] and ls[len(ls)-1][0] != 4 and inputing != True:
                ...
                cv2.putText(nowimg, 'CN', (x+20, y-20),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                cv2.rectangle(nowimg, (x+20, y-18),
                              (x+60, y-45), (255, 255, 255), 1)
                ls.append([8, x, y, 0])
            ...
            if ls == [] and inputing != True:
                ...
                nowimg = bgimg
                cv2.arrowedLine(
                    nowimg, (0, xyi[1]), (1600, xyi[1]), (0, 0, 0), 1, 8, 0, 0.02)  # x轴
                cv2.arrowedLine(
                    nowimg, (xyi[0], 900), (xyi[0], 0), (0, 0, 0), 1, 8, 0, 0.03)  # y轴
                cv2.putText(
                    nowimg, 'x', (1600-30, xyi[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                cv2.putText(
                    nowimg, 'y', (xyi[0], 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                # 标尺
                for i in [-6, -5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 6]:
                    sx = str(i)+'m'
                    sy = str(-i)+'m'
                    cv2.circle(
                        nowimg, (int(xyi[0]+100*i/Scalexy), xyi[1]), 2, (0, 0, 0), -1)
                    cv2.putText(nowimg, sx, (int(
                        xyi[0]+100*i/Scalexy), xyi[1]+25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)
                    cv2.circle(nowimg, (xyi[0], int(
                        xyi[1]+100*i/Scalexy)), 2, (0, 0, 0), -1)
                    cv2.putText(nowimg, sy, (xyi[0], int(
                        xyi[1]+100*i/Scalexy)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)
                ...
                cv2.putText(nowimg, 'home', (x+20, y+40),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                cv2.rectangle(nowimg, (x+20, y+15),
                              (x+110, y+45), (255, 255, 255), 1)
                ls.append([0, x, y, 0])
                lscom.append([0, height, 0, 0])
        elif flags == 4 and event == cv2.EVENT_MBUTTONDBLCLK:  # 中间键双击降落
            if ls != [] and ls[len(ls)-1][0] != 4 and inputing != True:
                prelist.append(nowimg.copy())
                cv2.circle(nowimg, (ls[len(ls)-1][1],
                                    ls[len(ls)-1][2]), 10, (0, 0, 255), -1)
                cv2.putText(nowimg, 'land', (ls[len(
                    ls)-1][1]-30, ls[len(ls)-1][2]+73), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                cv2.rectangle(nowimg, (ls[len(ls)-1][1]-30, ls[len(ls)-1][2]+50),
                              (ls[len(ls)-1][1]+37, ls[len(ls)-1][2]+75), (255, 255, 255), 1)
            ...
            if ls != [] and ls[len(ls)-1][0] != 4 and inputing != True:
                ...
                cv2.line(nowimg, (ls[-1][1]-15, ls[-1][2]+15),
                         (ls[-1][1]+15, ls[-1][2]-15), (0, 0, 0), 2, 4)
                cv2.putText(nowimg, 'holdon', (ls[len(
                    ls)-1][1]-118, ls[len(ls)-1][2]-15), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                cv2.rectangle(nowimg, (ls[-1][1]-118, ls[-1][2]-40),
                              (ls[len(ls)-1][1]-16, ls[len(ls)-1][2]-12), (255, 255, 255), 1)
            ...
            if ls != [] and ls[len(ls)-1][0] != 4 and ls[len(ls)-1][0] != 3 and ls[len(ls)-1][0] != 0 and ls[-1][1] != 0 and ls[-1][2] != 0 and inputing != True:
                ...
                if not posz.isdigit():
                    ...
                    posz = int(posz)
                print(posz)
                cv2.rectangle(nowimg, (x+10, y+10),
                              (x-10, y-10), (200, 0, 200), 1)
                ls.append([3, x, y, 0])
                ...
                if ls[i+1][0] != 4 and ls[i+1][0] != 1:
                    cv2.rectangle(
                        nowimg, (ls[i][1]+10, ls[i][2]+10), (ls[i][1]-10, ls[i][2]-10), (200, 0, 200), 1)
                    cv2.putText(
                        nowimg, 'gohome', (ls[i][1]-145, ls[i][2]+30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), fontbig)
                    cv2.rectangle(
                        nowimg, (ls[i][1]-145, ls[i][2]+4), (ls[i][1]-20, ls[i][2]+39), (255, 255, 255), 1)
                    cv2.arrowedLine(nowimg, (ls[i][1], ls[i][2]), ((
                        ls[i+1][1]+ls[i][1])//2, (ls[i+1][2]+ls[i][2])//2), (0, 255, 255), 2, 1, 0, 0.05)
                    cv2.line(nowimg, ((ls[i+1][1]+ls[i][1])//2, (ls[i+1][2]+ls[i]
                                                                 [2])//2), (ls[i+1][1], ls[i+1][2]), (0, 255, 255), 2, 4)
