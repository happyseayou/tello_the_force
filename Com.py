    ...
    def get_comd(self, kp, userc):
        ...
        if userc[4] == 1:
            ...
            if self.flymode == 0:  # flymode 0普通跟踪，只修正偏航
                self.pid_yaw = PID(0.25, 0, 0, setpoint=0,
                                   output_limits=(-100, 100))
                self.pid_thro = PID(
                    0.3, 0.005, 0.1, setpoint=0, output_limits=(-50, 50))
                ...
                self.pid_thro = PID(
                    0.3, 0.005, 0.1, setpoint=0, output_limits=(-50, 50))
                self.pid_pith = PID(
                    0.4, 0.04, 0.4, setpoint=0, output_limits=(-50, 50))

                comd[0] = int(-self.pid_yaw(xoff))
                comd[3] = int(self.pid_thro(yoff))

                if self.distance_midneck and self.lock_distance_mn:  # 判断是否存在这两个值，存在才能给命令，不存在则使用初始值
                    comd[2] = int(-self.pid_pith(self.lock_distance_mn -
                                                 self.distance_midneck))
                elif self.distance_shd and self.lock_distance_sd:
                    comd[2] = int(-self.pid_pith(self.lock_distance_sd -
                                                 self.distance_shd))
                ...
                self.pid_thro = PID(
                    0.3, 0.005, 0.1, setpoint=0, output_limits=(-50, 50))
                self.pid_pith = PID(
                    0.3, 0.01, 0.3, setpoint=0, output_limits=(-50, 50))
                self.pid_roll = PID(
                    0.2, 0.005, 0.2, setpoint=0, output_limits=(-30, 30))
                ...
                if self.isfly:
                    if self.pose == 0:  # 这层判断用于控制前后左右
                        ...
                        self.pid_yaw = PID(
                            0.25, 0, 0, setpoint=0, output_limits=(-100, 100))
                        comd[0] = int(self.pid_yaw(xoff))
                        comd[1] = -self.posespeed
                    elif self.pose == 4:
                        self.pid_yaw = PID(
                ...
                self.pid_thro = PID(
                    0.3, 0.005, 0.1, setpoint=0, output_limits=(-50, 50))
                self.pid_pith = PID(
                    0.3, 0.005, 0.1, setpoint=0, output_limits=(-30, 30))
